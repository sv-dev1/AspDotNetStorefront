...
{
    public partial class _Address : SkinBase
    {
        ...
        {
            if (ThisCustomer.IsRegistered == false && !CommonLogic.QueryStringBool("skipreg"))
            {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!this.IsPostBack)
            {
                ...
                if (addressErrorPrompt.Length > 0)
                {
                    ErrorMsgLabel.Visible = true;
                    ErrorMsgLabel.Text = addressErrorPrompt;
                }
            }
        ...
        private void LoadData()
        {
			...
			foreach (Address address in GetAddresses())
			{
                ...
                else addresses.Add(address); //show addresses that have no po boxes.
			}
        ...
        protected void dlAddress_UpdateCommand(object sender, DataListCommandEventArgs e)
        {
            ...
            if (AppLogic.AppConfig("VerifyAddressesProvider") != "")
            {
                ...
                Verifyaddress.Address1 = ctrlAddress.Address1;
                Verifyaddress.Address2 = ctrlAddress.Address2;
                Verifyaddress.City = ctrlAddress.City;
                Verifyaddress.State = ctrlAddress.State;
                Verifyaddress.Zip = ctrlAddress.ZipCode;

                VerifyResult = AddressValidation.RunValidate(Verifyaddress, out StandardizedAddress);

                if (VerifyResult != AppLogic.ro_OK)
                {
                    lbAddressError.Text += VerifyResult; //lovely Ecom Added
                    return;
                }
            }
            //LovelyEcom end
        ...
        protected void dlAddress_ItemCommand(object sender, DataListCommandEventArgs e)
        {
            if (e.CommandArgument.Equals("MakePrimaryAddress"))
            {
                ...
                StringBuilder sSqlPO = new StringBuilder(5000);
                bool isPOBoxAddress = false;
                lblPOBoxError.Visible = false;

                if (Int32.TryParse((e.Item.FindControl("hfAddressID") as HiddenField).Value, out addressID))
                {
                    if (AppLogic.AppConfigBool("DisallowShippingToPOBoxes") && (AddressMode == AddressTypes.Shipping))
                    {
                        using (SqlConnection con = new SqlConnection(DB.GetDBConn()))
                        {
                            sSqlPO.AppendFormat("SELECT Address.Address1 FROM Address WHERE CustomerID={0} AND Address.AddressID = {1}", ThisCustomer.CustomerID, addressID);
                            con.Open();
                            using (IDataReader rs = DB.GetRS(sSqlPO.ToString(), con))
                            {
                                while (rs.Read())
                                {
                                    string tempAddress1 = DB.RSField(rs, "Address1");
                                    isPOBoxAddress = CheckPOBox(tempAddress1);
                                }
                            }
                        }
                    }
                    ...
                    if (!isPOBoxAddress)
                    {
                        ...
                        {
                            sSql.AppendFormat("UPDATE Customer SET BillingAddressID={0}, ShippingAddressID={1}, BillingEqualsShipping=1 WHERE CustomerID={2}", addressID, addressID, ThisCustomer.CustomerID);
                        ...
                        if (AppLogic.AppConfigBool("AllowShipToDifferentThanBillTo"))
                        {
                            bool billingEqualsShipping = (AddressMode == AddressTypes.Shipping && ThisCustomer.PrimaryBillingAddressID == addressID) ||
                                ((AddressMode == AddressTypes.Billing || AddressMode == AddressTypes.Unknown) && ThisCustomer.PrimaryShippingAddressID == addressID);
                            DB.ExecuteSQL("update Customer set BillingEqualsShipping = @BilllingEqualsShipping where CustomerId = @CustomerId",
                                new SqlParameter[] { new SqlParameter("@BilllingEqualsShipping", (billingEqualsShipping ? 1 : 0)), 
                    ...
                    {
                        //dont switch shipping address if contains POBox
                        lblPOBoxError.Visible = true;
        ...
        private String DisplayPaymentMethod(int primaryID)
        {
            ...
            {
                paymentMethodDisplay = String.Format("<nobr>ECheck - {0}: {1} {2}", primaryAddress.ECheckBankName, primaryAddress.ECheckBankABACodeMasked, primaryAddress.ECheckBankAccountNumberMasked) + "</nobr><br/>";
            }
        ...
        private void DisableEditButtonsForAddressWithOpenOrder(ImageButton deleteButton, ImageButton editButton, int addressID)
        {
