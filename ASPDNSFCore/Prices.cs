...
namespace AspDotNetStorefrontCore
{
    ...
    public partial class Prices
    {
        ...
        public static Decimal VariantPrice(Customer ThisCustomer, int variantID, decimal regularPrice, decimal salePrice, decimal extPrice, decimal attributesPriceDelta, Boolean returnDiscountedPrice, int taxClassID)
        {
            ...
            Boolean VATEnabled = AppLogic.ProductIsMLExpress() == false && AppLogic.AppConfigBool("VAT.Enabled");
        ...
        public static Decimal TaxRate(Customer ThisCustomer, int TaxClassID)
        {
            ...
            if ("billing".Equals(AppLogic.AppConfig("TaxCalcMode"), StringComparison.InvariantCultureIgnoreCase))
            {
                ...
                return TaxRate(ThisCustomer.PrimaryBillingAddress, TaxClassID);
            }
            else // TaxCalcMode is shipping
            {
                // use the shipping address to get the tax rate
                return TaxRate(ThisCustomer.PrimaryShippingAddress, TaxClassID);
            }
        }
        ...
        /// <param name="TaxClassID">The item tax class id</param>
        /// <returns></returns>
        public static Decimal TaxRate(Address useAddress, int TaxClassID)
        {
            Decimal rate = System.Decimal.Zero;

            //sets default vat country id
            int CountryID = CommonLogic.IIF(AppLogic.AppConfigBool("VAT.Enabled") && AppLogic.ProductIsMLExpress() == false, AppLogic.AppConfigUSInt("VAT.CountryID"), 0);
            int StateID = 0;
            ...
            rate = TaxRate(CountryID, StateID, ZipCode, TaxClassID, Customer.Current);
		...
		public static Decimal TaxTotal(Customer thisCustomer, CartItemCollection cartItems, Decimal shipCost, IEnumerable<OrderOption> orderOptions)
		{
			...
			try
			{
				...
				// Apply addins
				if(AppLogic.UseAddIn<ITaxRates>((tRates) => { TaxRatesHook(tRates, thisCustomer, cartItems); }))
					return ThisTaxTotal;

               //LovelyEcom apply taxcloud if enabled
                if (AppLogic.AppConfigBool("taxcloud.Enabled"))
                {
                    TaxCloud taxC = new TaxCloud();
                    return taxC.GetTaxRate(thisCustomer, cartItems, orderOptions);
                }
			...
			{
				SysLog.LogException(Ex, MessageTypeEnum.AddinInvocation, MessageSeverityEnum.Error);
			}
			...
			{
				taxAmount += ShippingTax(shipCost, shipAddresses.First());
			}
			else
			{
				foreach(int addr in shipAddresses)
				{
					...
					taxAmount += ShippingTax(shipCost, addr);
				}
			}
        ...
        /// <returns></returns>
        public static decimal ShippingTax(Decimal shipCost, int addressID)
        {
            Address addr = new Address();
            addr.LoadFromDB(addressID);

            return ShippingTax(shipCost, addr);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public static decimal ShippingTax(Decimal shipCost, Address shipAddr)
        {
            Decimal tax = System.Decimal.Zero;

            tax = shipCost * (TaxRate(shipAddr, AppLogic.AppConfigUSInt("ShippingTaxClassID")) / 100.0M);
		...
		public static decimal ShippingTotalForAddress(bool includeDiscount, bool includeTax, CartItemCollection cartItems, Customer customer, IEnumerable<OrderOption> orderOptions, int destinationAddressId, int numberOfShipmentsInOrder)
		{
			...
			bool vatOn = !AppLogic.ProductIsMLExpress() && AppLogic.AppConfigBool("VAT.Enabled") && customer.VATSettingReconciled == VATSettingEnum.ShowPricesInclusiveOfVAT;
			...
			else if(!Shipping.NoShippingRequiredComponents(cartItemsForAddress))
			{
							...
							try
							{
								...
								using(SqlConnection conn = new SqlConnection(DB.GetDBConn()))
								{
									...
									using(IDataReader rs = DB.GetRS(query, conn))
									{
										if(rs.Read())
										{
											shipPercent = (decimal)DB.RSFieldDecimal(rs, "FixedPercentOfTotal");
										}
									}
								}
							}
				...
				if(vatOn && includeTax)
				{
					...
					VAT = ShippingTax(ThisAddressShipCost, shipAdr);
				}
				shippingCost += VAT;
			}
        ...
        static public Decimal GetUpsellProductPrice(int SourceProductID, int UpsellProductID, int CustomerLevelID)
        {
            ...
            Decimal PR = AppLogic.DetermineLevelPrice(AppLogic.GetProductsFirstVariantID(UpsellProductID), CustomerLevelID, out IsOnSale);
        ...
        {
            bool m_VATEnabled = AppLogic.ProductIsMLExpress() == false && AppLogic.AppConfigBool("VAT.Enabled");
            ...
            {
                if (ThisCustomer.CustomerLevelID == 0 || ThisCustomer.LevelDiscountPct == 0.0M)
                {
