...
namespace AspDotNetStorefrontGateways
{
    ...
    public partial class Gateway
    {
        ...
        //this is not actually used for display - it is used for google checkout logic and should not be removed or changed
        static public readonly String ro_GWGOOGLECHECOUTForDisplay = "Google Checkout";
        ...
        public static Decimal MaxMindFraudCheck(int OrderNumber, Customer ThisCustomer, Address UseBillingAddress, Address UseShippingAddress, out String FraudDetails)
        {
            ...
            try
            {
                ...
                String SessionID = "aspdnsf"; // MaxMind requires this value to identify our cart, do not change
                ...
                String RequestIP = CommonLogic.CustomerIpAddress();
                if (ThisIP.Length == 0)
                {
                    ThisIP = RequestIP;
                }
                ...
                {
                    CN = UseBillingAddress.CardNumber.Substring(0, 6);
                }
                MaxMindAPI.MINFRAUD rsp = mmind.minfraud_soap(ThisIP, UseBillingAddress.City, UseBillingAddress.State, UseBillingAddress.Zip, UseBillingAddress.Country, BillingEMailDomain, CN, binName, binPhone, UseBillingAddress.Phone, AppLogic.AppConfig("MaxMind.LicenseKey"), RequestedType, ForwardedIP, EMailMD5, UsernameMD5, PasswordMD5, UseShippingAddress.Address1, UseShippingAddress.City, UseShippingAddress.State, UseShippingAddress.Zip, UseShippingAddress.Country, TXNID, SessionID);
        ...
        public static void ProcessOrderAsCaptured(int OrderNumber)
        {
            ...
            if (!ord.TransactionIsCaptured())
            {
                ...
                //Update the Micropay balances if any was purchased
                {
                    ...
                    int MicropayVariantID = AppLogic.GetProductsFirstVariantID(MicropayProductID);
                ...
                if (ord.HasDownloadComponents(true) && ord.DownloadEMailSentOn == System.DateTime.MinValue && !AppLogic.AppConfigBool("DelayedDownloads") && ord.EMail.Length > 0)
                {
                    bool downloadListSent = false;
                    foreach (CartItem i in ord.CartItems)
                    {
                        if (i.IsDownload)
                        {
                            // only send download list once for non SA items, as ord.getdownloadlist(true) sends a links to ALL non SA items at one time:
                            if (!downloadListSent)
                            {
                                try
                                {
                                    String SubjectReceipt = String.Format(AppLogic.GetString("common.cs.1", ord.SkinID, ord.LocaleSetting), AppLogic.AppConfig("StoreName"));
                                    AppLogic.SendMail(SubjectReceipt.Replace(AppLogic.GetString("common.cs.7", ord.SkinID, ord.LocaleSetting), AppLogic.GetString("common.cs.8", ord.SkinID, ord.LocaleSetting)), ord.GetDownloadList(true), true, AppLogic.AppConfig("ReceiptEMailFrom"), AppLogic.AppConfig("ReceiptEMailFromName"), ord.BillingAddress.m_EMail, ord.BillingAddress.m_EMail, "", AppLogic.MailServer());
                                }
                                catch { }
                    ...
                    if (ord.HasDownloadComponents(true))
                    {
                        DB.ExecuteSQL("update Orders set " + CommonLogic.IIF(ord.IsAllDownloadComponents(), "IsNew=0,", "") + " DownloadEMailSentOn=getdate()" + CommonLogic.IIF(ord.IsAllDownloadComponents(), ", ShippedOn=getdate(), ShippedVIA='DOWNLOAD'", "") + " where OrderNumber=" + ord.OrderNumber.ToString());
                    }
                }
                ...
                AppLogic.UseAddIn<IPackOrder>(packAddin => OrderPackingHook(packAddin, ord, true));
                ...
                #region lovelyEcom add
                if (AppLogic.AppConfigBool("taxcloud.Enabled"))
                {
                    TaxCloud taxcloud = new TaxCloud();
                    taxcloud.CommitTax(ord);
                }
                #endregion

                AppLogic.UseAddIn<ITaxRates>((tRates) => { CommitTax(tRates, ord); });
            }
        }
        ...
        public static int CreateOrderRecord(ShoppingCart cart, int OrderNumber, Address UseBillingAddress)
        {
            ...
            using (SqlConnection con = new SqlConnection(DB.GetDBConn()))
            {
                ...
                using (IDataReader rsCustomer = DB.GetRS("select * from customer  with (NOLOCK)  where customerid=" + cart.ThisCustomer.CustomerID.ToString(), con))
                {
                    ...
                    bool AllowAnonymousCheckout = AppLogic.AppConfigBool("PasswordIsOptionalDuringCheckout");
        ...
        public static String MakeAdHocOrder(String PaymentGatewayToUse, int OriginalOrderNumber, String OriginalTransactionID, Customer OrderCustomer, Address UseBillingAddress, String CardExtraCode, Decimal OrderTotal, AppLogic.TransactionTypeEnum OrderType, String OrderDescription, out int NewOrderNumber)
        {
            ...
            {
                status = Gateway.ProcessCard(null, PaymentGatewayToUse, AppLogic.ro_TXModeAuthCapture, OrderCustomer.CustomerID, NewOrderNumber, UseBillingAddress, CardExtraCode, null, OrderTotal, AppLogic.AppConfigBool("UseLiveTransactions"), String.Empty, String.Empty, String.Empty, out AVSResult, out AuthorizationResult, out AuthorizationCode, out AuthorizationTransID, out TransactionCommand, out TransactionResponse, out GWCleaned);
                if (AVSResult == null)
                {
            ...
            if (status == AppLogic.ro_OK)
            {
                ...
                if (!IsRefund)
                {
                    DB.ExecuteSQL("update orders set IsNew=0, TransactionState=" + DB.SQuote(AppLogic.ro_TXStateCaptured) + ", AuthorizedOn=getdate(), CapturedOn=getdate() where OrderNumber=" + NewOrderNumber.ToString());
                }
                else
                {
        ...
        public static String MakeOrder(String PaymentGatewayToUse, String TransactionMode, ShoppingCart cart, int OrderNumber, String CAVV, String ECI, String XID, String RecurringSubscriptionID)
        {
            ...
            else
            {
                ...
                if (AppLogic.AppConfigBool("MaxMind.Enabled") && !IsBeingImpersonated)
                {
                    ...
                    FraudScore = MaxMindFraudCheck(OrderNumber, cart.ThisCustomer, UseBillingAddress, UseShippingAddressX, out FraudDetails);
                }
                //end Max Mind check
                ...
                if (PM == AppLogic.ro_PMCreditCard)
                {
                    ...
                    if (maxmindstatus == AppLogic.ro_OK)
                    {
                        ...
                        status = ProcessCard(cart, PaymentGatewayToUse, TransactionMode, cart.ThisCustomer.CustomerID, OrderNumber, UseBillingAddress, CardExtraCode, UseShippingAddress, OrderTotal, AppLogic.AppConfigBool("UseLiveTransactions"), CAVV, ECI, XID, out  AVSResult, out  AuthorizationResult, out  AuthorizationCode, out  AuthorizationTransID, out  TransactionCommand, out TransactionResponse, out GW);
                ...
                {
                    status = ProcessCard(cart, PaymentGatewayToUse, TransactionMode, cart.ThisCustomer.CustomerID, OrderNumber, UseBillingAddress, String.Empty, UseShippingAddress, OrderTotal, AppLogic.AppConfigBool("UseLiveTransactions"), CAVV, ECI, XID, out  AVSResult, out  AuthorizationResult, out  AuthorizationCode, out  AuthorizationTransID, out  TransactionCommand, out TransactionResponse, out GW);
            ...
            if (status == AppLogic.ro_OK)
            {
                ...
                String sql4 = "INSERT INTO orders_ShoppingCart(OrderNumber,DistributorID,CartType,ShippingMethodID,ShippingMethod,"
                    ...
                    + " OrderedProductPrice,CustomerEntersPrice,IsTaxable,IsShipSeparately,IsDownload,DownloadLocation,FreeShipping, IsAKit, IsAPack,"
                    + " IsSystem, TaxClassID, TaxRate, IsGift)"
                    ...
                    + " sc.IsDownload,sc.DownloadLocation,sc.FreeShipping, sc.IsAKit, sc.IsAPack, sc.IsSystem, sc.TaxClassID,"
                    + "(isnull(cr.taxrate, 0)+isnull(sr.taxrate, 0)+isnull(zr.taxrate, 0)), IsGift ", OrderNumber)
                ...
                String sql6 = "insert into orders_customcart(OrderNumber,CartType,CustomCartRecID,CustomerID,PackID,ShoppingCartRecID,ProductSKU,ProductName,productVariantName,ProductWeight,ProductID,VariantID,Quantity,ChosenColor,ChosenColorSKUModifier,ChosenSize,ChosenSizeSKUModifier)"
                    + String.Format(" select {0},CartType,CustomCartRecID,CustomerID,PackID,ShoppingCartRecID,ProductSKU,Product.Name,ProductVariant.Name,ProductWeight,product.ProductID,productvariant.VariantID,Quantity,ChosenColor,ChosenColorSKUModifier,ChosenSize,ChosenSizeSKUModifier", OrderNumber)
                    + String.Format(" from ((customcart left outer join product  with (NOLOCK)  on customcart.productid=product.productid) left outer join productvariant  with (NOLOCK)  on customcart.variantid=productvariant.variantid) "
                    + " where CartType={0} and customerid={1} and ShoppingCartRecID <> 0", (int)cart.CartType, cart.ThisCustomer.CustomerID)
                    + RecurringOrderSql;
                DB.ExecuteSQL(sql6);
                ...
                if (cart.CartType == CartTypeEnum.ShoppingCart)
                {
                    ...
                    DB.ExecuteSQL("delete from customcart where CartType=" + ((int)CartTypeEnum.ShoppingCart).ToString() + CommonLogic.IIF(RecurringVariantsList.Length != 0, " and VariantID not in (" + RecurringVariantsList + ")", "") + " and customerid=" + cart.ThisCustomer.CustomerID.ToString());
                ...
                if (RecurringVariantsList.Length != 0 && cart.ContainsRecurringAutoShip)
                {
                    ...
                    if (cart.OriginalRecurringOrderNumber == 0)
                    {
                        ...
                        if (AppLogic.AppConfigBool("Recurring.UseGatewayInternalBilling") && RecurringSubscriptionID.Length == 0)
                        {
                            ...
                            String SetupStatus = String.Empty;
                            ...
                            if (processor != null)
                            {
                                SetupStatus = processor.RecurringBillingCreateSubscription(firstcartrecurringitem.ProductName,
                                                    cart.ThisCustomer,
                                                    UseBillingAddress,
                                                    UseShippingAddress,
                                                    RecurringAmount,
                            ...
                            {
                                // TBD. what to do here if SetupStatus is not OK??
                ...
                // call-out order packing add-ins
                if (AppLogic.AppConfigBool("AddIns.Enabled") || AppLogic.AppConfigBool("AvalaraTax.Enabled") || AppLogic.AppConfigBool("taxcloud.Enabled"))//LovelyEcom add
                {
                    ...
                    {
                        TaxCloud taxcloud = new TaxCloud();
                        taxcloud.OrderPlaced(order);
                    }
                    #endregion

                    if (AppLogic.AppConfigBool("AddIns.Enabled"))
						AppLogic.UseAddIn<ITaxRates>((tRates) => { OrderPlaced(tRates, order); });
                }

                if (SetToCapturedState && GW != ro_GWGOOGLECHECKOUT)
                {
        ...
        private static String ProcessCard(ShoppingCart cart, String GateWay, String TransactionMode, int CustomerID, int OrderNumber, Address UseBillingAddress, String CardExtraCode, Address UseShippingAddress, Decimal OrderTotal, bool useLiveTransactions, String CAVV, String ECI, String XID, out String AVSResult, out String AuthorizationResult, out String AuthorizationCode, out String AuthorizationTransID, out String TransactionCommand, out String TransactionResponse, out String GatewayUsed)
        {
            ...
            {
                Status = PayPalController.ProcessPaypal(OrderNumber, CustomerID, OrderTotal, useLiveTransactions, TransactionMode, UseBillingAddress, UseShippingAddress, CAVV, ECI, XID, out AVSResult, out AuthorizationResult, out  AuthorizationCode, out  AuthorizationTransID, out  TransactionCommand, out TransactionResponse);
            }
            else
            {
        ...
        public static String DispatchCapture(String GW, int OrderNumber)
        {
            ...
            {
                if (GW.StartsWith(ro_GWGOOGLECHECKOUT, StringComparison.InvariantCultureIgnoreCase))
                {
                    GW = ro_GWGOOGLECHECKOUT;  // strip-off the GOrderNumber
                }
                else if (GW == ro_GWGOOGLECHECKOUT)
                {
                    Status = GoogleCheckout.CaptureOrder(o);
                }
                //else if (GW == ro_GWAMAZONSIMPLEPAY)
                //{
                //    Status = AmazonSimplePay.CaptureOrder(o);
                //}
                else
                {
        ...
        public static String ProcessRefund(int CustomerID, int OriginalOrderNumber, int NewOrderNumber, Decimal RefundAmount, String RefundReason, Address UseBillingAddress)
        {
            ...
            {
                if (GW == ro_GWGOOGLECHECKOUT)
                {
                    Status = GoogleCheckout.RefundOrder(OriginalOrderNumber, NewOrderNumber, RefundAmount, RefundReason, UseBillingAddress);
                }
                else
                {
            ...
            if (Status.ToUpper(CultureInfo.InvariantCulture) == AppLogic.ro_OK && GW != ro_GWGOOGLECHECKOUT)
            {
                ...
                if ((RefundAmount == System.Decimal.Zero || RefundAmount == OrderTotal))
                {
                    ...
                    if (AppLogic.AppConfigBool("taxcloud.Enabled"))
                    {
                        ...
                        TaxCloud taxcloud = new TaxCloud();
                        taxcloud.IssueRefund(new Order(OriginalOrderNumber), originAddress, 0);
                    }
                    #endregion
                    // Send a Refund Tax Transaction 
                    AppLogic.UseAddIn<ITaxRates>((tRates) => { IssueRefund(tRates, new Order(OriginalOrderNumber)); });
                }
                else
                {
					if(AppLogic.AppConfigBool("AvalaraTax.Enabled"))
                    ...
                    if (AppLogic.AppConfigBool("taxcloud.Enabled"))
                    {
                        ...
                        TaxCloud taxcloud = new TaxCloud();
                        taxcloud.IssueRefund(new Order(OriginalOrderNumber), originAddress, RefundAmount);
                    }
                    #endregion

                    // Send a Refund Tax Transaction 
                    AppLogic.UseAddIn<ITaxRates>((tRates) => { IssueRefund(tRates, new Order(OriginalOrderNumber), RefundAmount); });
                }
            }
        ...
        public static String DispatchVoid(String GW, int OrderNumber)
        {
            ...
            else
            {
                if (GW == ro_GWGOOGLECHECKOUT)
                {
                    Status = GoogleCheckout.VoidOrder(OrderNumber);
                }
                else
                {
            ...
            if (GW.StartsWith(ro_GWGOOGLECHECKOUT, StringComparison.InvariantCultureIgnoreCase) &&
                Status.Equals(AppLogic.ro_OK, StringComparison.InvariantCultureIgnoreCase))
            {
                Status = "Void OK. Google Checkout will send cancellation E-Mail to the customer.";
            }
            return Status;
        }
        ...
        public static String StartExpressCheckout(ShoppingCart cart, Address ShippingAddress)
        {
            return StartExpressCheckout(cart, ShippingAddress, false);
        }

        public static String StartExpressCheckout(ShoppingCart cart, Address ShippingAddress, bool boolBypassOrderReview)
        {
            switch (PayPalController.GetAppropriateExpressType())
            {
                case ExpressAPIType.PayFlowPro:
                    return PayFlowProController.StartEC(cart, ShippingAddress, boolBypassOrderReview);
                case ExpressAPIType.PayPalExpress:
                case ExpressAPIType.PayPalAcceleratedBording:
                case ExpressAPIType.NoValidAPIType:
                default:
                    return PayPalController.StartEC(cart, ShippingAddress, boolBypassOrderReview);
            }
        }
        ...
        public static String OrderManagement_DoVoid(Order ord, String ViewInLocaleSetting, bool force)
        {
            ...
            else
            {
                ...
                if (AppLogic.AppConfigBool("taxcloud.Enabled"))
                {
                    ...
                    TaxCloud taxcloud = new TaxCloud();
                    taxcloud.VoidTax(ord);
                }
                #endregion
                // add call to commit Tax transactions if available.
                AppLogic.UseAddIn<ITaxRates>((tRates) => { VoidTax(tRates, ord); });
            }
        ...
        static public String OrderManagement_DoForceFullRefund(Order ord, String ViewInLocaleSetting)
        {
            ...
            else
            {
                ...
                if (AppLogic.AppConfigBool("taxcloud.Enabled"))
                {
                    ...
                    TaxCloud taxcloud = new TaxCloud();
                    taxcloud.IssueRefund(ord, originAddress, 0);
                }
                #endregion
                // add call to Refund Tax transactions or this order
                AppLogic.UseAddIn<ITaxRates>((tRates) => { IssueRefund(tRates, ord); });
            }
            return Status;
        }
...
#region LovelyEcom Add
                ...
                else
                {
                    ...
                    #endregion
                    // add call to void Tax transactions if available.
                    AppLogic.UseAddIn<ITaxRates>((tRates) => { VoidTax(tRates, ord); });
                }
            }
        ...
        static public String OrderManagement_SendDownloadNotification(Order ord, String ViewInLocaleSetting)
        {
            String Status = AppLogic.ro_OK;
            if (ord.OrderNumber == 0 || ord.IsEmpty)
            {
            ...
            else
            {
                ...
                {
                    Customer c = new Customer(ord.CustomerID, true);
                    Order.MarkOrderAsShipped(ord.OrderNumber, "DOWNLOAD", "", DateTime.Now, false, null, new Parser(null, 1, c), true);
                    if (ord.PaymentGateway.StartsWith(Gateway.ro_GWGOOGLECHECOUTForDisplay))
                    {
                        GoogleCheckout.DeliverOrder(ord.OrderNumber, "DOWNLOAD", "", false);
                        GoogleCheckout.ArchiveOrder(ord.OrderNumber);
                    }
                }

                if (ord.EMail != "" && ord.HasDownloadComponents(true) && ord.ThereAreDownloadFilesSpecified())
                {
                    String SubjectReceipt = String.Empty;
                    if (AppLogic.AppConfigBool("UseLiveTransactions"))
                    {
                        SubjectReceipt = AppLogic.AppConfig("StoreName") + " Receipt";
                    }
                    else
                    {
                        SubjectReceipt = AppLogic.AppConfig("StoreName") + " Receipt (TEST)";
                    }
                        ...
                        MailServer.Equals(AppLogic.ro_TBD, StringComparison.InvariantCultureIgnoreCase) == false)
                    {
                        ...
                        int ThisTry = 1;
                        bool SentOK = false;
                        while (ThisTry <= MaxTries && !SentOK)
                        {
                            try
                            {
                                AppLogic.SendMail(SubjectReceipt.Replace("Receipt", "Download Instructions"), ord.GetDownloadList(true), true, AppLogic.AppConfig("ReceiptEMailFrom"), AppLogic.AppConfig("ReceiptEMailFromName"), ord.EMail, ord.EMail, String.Empty, MailServer);
                                DB.ExecuteSQL("update Orders set DownloadEMailSentOn=getdate(), IsNew=0 " + CommonLogic.IIF(ord.IsAllDownloadComponents(), ", ShippedOn=getdate(), ShippedVIA='DOWNLOAD'", "") + " where OrderNumber=" + ord.OrderNumber.ToString());
                                SentOK = true;
                            }
                            catch { }
                            ThisTry++;
                ...
                {
                    Status = "NO DOWNLOAD ITEMS OR NO DOWNLOAD FILES SPECIFIED OR  NO CUSTOMER E-MAIL ADDRESS FOUND. DOWNLOAD E-MAIL NOT SENT.";
                }
            }
            return Status;
        }

        static public String OrderManagement_SimulateDownloadNotification(Order ord, String ViewInLocaleSetting)
        {
            String Status = AppLogic.ro_OK;
            if (ord.OrderNumber == 0 || ord.IsEmpty)
            {
            ...
            {
                DB.ExecuteSQL("update Orders set DownloadEMailSentOn=getdate() " + CommonLogic.IIF(ord.IsAllDownloadComponents(), ", ShippedOn=getdate(), ShippedVIA='DOWNLOAD'", "") + " where OrderNumber=" + ord.OrderNumber.ToString());
            }
            return Status;
        }
        ...
        static public String OrderManagement_SendDistributorNotification(Order ord, String ViewInLocaleSetting)
        {
            ...
            else
            {
                ...
                if (ord.IsAllDownloadComponents() && ord.isAllDistributorComponents())
                {
                    ...
                    if (ord.PaymentGateway.StartsWith(Gateway.ro_GWGOOGLECHECOUTForDisplay))
                    {
                        GoogleCheckout.DeliverOrder(ord.OrderNumber, "DISTRIBUTOR", "", false);
                        GoogleCheckout.ArchiveOrder(ord.OrderNumber);
                    }
                }
            }
            ...
            else
            {
                ...
                if (DB.GetSqlN("select count(*) as N from Customer  with (NOLOCK)  where EMail=" + DB.SQuote(NewEMail) + " and Deleted=0") == 1)
                {
                    ...
                    if (CustomerID != 0)
                    {
                        ...
                        DB.ExecuteSQL("update Orders_CustomCart set CustomerID=" + CustomerID.ToString() + " where OrderNumber=" + ord.OrderNumber.ToString());
                    }
                }
                else
                {
        ...
        static public String OrderManagement_MarkAsReadyToShip(Order ord, String ViewInLocaleSetting)
        {
            ...
            else
            {
                if (ord.PaymentGateway.StartsWith(Gateway.ro_GWGOOGLECHECOUTForDisplay))
                {
                    // Google Checkout Orders only, mark it as "in process" state:
                    GoogleCheckout.ProcessOrder(ord);
                }
                DB.ExecuteSQL("update Orders set ReadyToShip=1 where OrderNumber=" + ord.OrderNumber.ToString());

                AppLogic.UseAddIn<IFulfillment>(fulfillmentAddin => OrderFulfillmentHook(fulfillmentAddin, ord));
            }
            return Status;
        }
        ...
        static public String OrderManagement_MarkAsShipped(Order ord, String ViewInLocaleSetting, String ShippedVIA, String TrackingNumber, DateTime ShippedOn)
        {
            ...
            {
                //v7 For Google Checkout, we inhibit a shipped notification email from us since Google handles it.
                ...
                if (ord.PaymentGateway.StartsWith(Gateway.ro_GWGOOGLECHECOUTForDisplay))
                {
                    Order.MarkOrderAsShipped(ord.OrderNumber, ShippedVIA, TrackingNumber, ShippedOn, false, null, new Parser(null, 1, c), true);

                    string GoogleCarrier = string.Empty;
                    if (ShippedVIA.Length >= 2)
                    {
                        switch (ShippedVIA.ToUpperInvariant().Trim().Substring(0, 2))
                        {
                            case "DH":
                                GoogleCarrier = "DHL";
                                break;
                    ...
                    if (TrackingNumber.Length > 0 && GoogleCarrier.Length == 0)
                    {
                        GoogleCarrier = "Other";
                    }
                    GoogleCheckout.DeliverOrder(ord.OrderNumber, GoogleCarrier, TrackingNumber, true);
                    GoogleCheckout.ArchiveOrder(ord.OrderNumber);
                }
                else
                {
        ...
        static public String OrderManagement_SetCustomerServiceNotes(Order ord, String ViewInLocaleSetting, String Notes)
        {
            ...
            else
            {
                ...
                if (ord.PaymentGateway.StartsWith(Gateway.ro_GWGOOGLECHECOUTForDisplay))
                {
                    // Google Checkout Orders only
                    GoogleCheckout.SendBuyerMessage(ord.OrderNumber, Notes, false);
                }
            }
            return Status;
        }
        ...
        static public String OrderManagement_SetTracking(Order ord, String ViewInLocaleSetting, String ShippedVIA, String TrackingNumber)
        {
            ...
            else
            {
                ...
                if (ord.PaymentGateway.StartsWith(Gateway.ro_GWGOOGLECHECOUTForDisplay))
                {
                    ...
                    GoogleCheckout.AddTrackingData(ord.OrderNumber, GoogleCarrier, TrackingNumber);
                }
            }
            return Status;
        }
